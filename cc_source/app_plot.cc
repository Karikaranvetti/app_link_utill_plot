...
Napi::Object app_plot::Init(Napi::Env env, Napi::Object exports) {
    Napi::Function func =DefineClass(env,"app_plot",{
...
Napi::Value app_plot::start(const Napi::CallbackInfo& info) {
    ...
    /*commit thread*/
    std::thread th_pub;
    th_pub = std::thread([this] {app_plot_req_pub_thread(); });
    th_pub.detach();

    std::thread th_sub;
    th_sub = std::thread([this] {app_plot_data_sub_thread(); });
    th_sub.detach();

    std::thread th_process;
    th_sub = std::thread([this] {app_plot_data_process_thread(); });
    th_sub.detach();
